...
public class CommonFrameLayout extends FrameLayout {
    ...
    public CommonFrameLayout(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initlialize(attrs);
    }

    private void initlialize(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.CommonFrameLayout);
        try {
            mBorderDrawble = ta.getDrawable(R.styleable.CommonFrameLayout_border_drawable);
            mTouchedDrawble = ta.getDrawable(R.styleable.CommonFrameLayout_touched_drawable);
            mCornerRadius = ta.getDimension(R.styleable.CommonFrameLayout_corner_radius, 0);
        }catch (Throwable t){
            t.printStackTrace();
        }finally {
            ta.recycle();
